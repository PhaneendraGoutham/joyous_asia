...
$(document).ready(function(){
	...
	$("#readonlySize").val($("#size").val());
	$("#readonlyTransferAmount").val($("#transferAmount").val());
	
	//to set default start date and end date = today's date
	var rentalFromDate = $("#rentalFrom").val();
	var rentalToDate = $("#rentalTo").val();
	
	//set min date and max date by default
	$('#datetimePickerStartDate').data("DateTimePicker").maxDate(moment(rentalToDate, 'DD-MM-YYYY').toDate());
	$('#datetimePickerEndDate').data("DateTimePicker").minDate(moment(rentalFromDate, 'DD-MM-YYYY').toDate());
	...
	var timeArray = $("#returnGownFromtimeTotime").val().split("-");
	$("#timeFrom").val(timeArray[0].trim());
	$("#timeTo").val(timeArray[1].trim());
	...
	payBySelector($("#payByDetails").val());
	rentalReturnDetailSelector($("#rentalReturnDetails").val());
	
	if($("#jsReceiptNum").val() != "" && $('#jsReceiptNum').val() != null){
		$('#deposit_transfer_into_js').iCheck("check");
		$(".jsRelativeDiv").show();
	}
...
//		},
	...
	$(".btn-quantity").on("click", function(e) {
		...
		if(btnIdArray[0]=="plus-btn"){
			console.log(parseInt($("#item_"+btnIdArray[1]).val()));
